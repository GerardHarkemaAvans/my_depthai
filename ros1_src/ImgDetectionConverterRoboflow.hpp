...
#endif
class ImgDetectionConverterRoboflow {
   public:
    // DetectionConverter() = default;
    ImgDetectionConverterRoboflow(std::string frameName, int width, int height, bool normalized = false);

    void toRosMsg(std::shared_ptr<dai::ImgDetectionsRoboflow> inNetData, std::deque<VisionMsgs::Detection2DArray>& opDetectionMsgs);

    Detection2DArrayPtr toRosMsgPtr(std::shared_ptr<dai::ImgDetectionsRoboflow> inNetData);
