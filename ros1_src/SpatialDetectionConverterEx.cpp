/**
	SpatialDetectionConverterEx.cpp
...
SpatialDetectionConverterEx::SpatialDetectionConverterEx(std::string frameName, int width, int height, bool normalized)
    : _frameName(frameName), _width(width), _height(height), _normalized(normalized), _steadyBaseTime(std::chrono::steady_clock::now()) {}

void SpatialDetectionConverterEx::toRosMsg(std::shared_ptr<dai::SpatialImgDetections> inNetData,
    ...
    // publishing
    int width_offset = (_width - _height) / 2;
    for(int i = 0; i < inNetData->detections.size(); ++i) {
        ...
        if(_normalized) {
            ...
            xMin = (inNetData->detections[i].xmin * _height) + width_offset;
            yMin = inNetData->detections[i].ymin * _height;
            xMax = (inNetData->detections[i].xmax * _height) + width_offset;
...
SpatialDetectionArrayPtr SpatialDetectionConverterEx::toRosMsgPtr(std::shared_ptr<dai::SpatialImgDetections> inNetData) {
